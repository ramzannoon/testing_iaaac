...
class SMSClient(models.Model):
    ...
    def send_msg(self, std, data):
        ...
        if gateway:
            ...
            url = gateway.url
            if std._name == 'crm.lead':
                mobile = std.phone
            else:
                mobile = std.mobile
            partner_ids = std.message_follower_ids
            params = f"key={gateway.token_id}" + "&" + f"sender={gateway.sender}" + "&" + f"receiver={mobile}" + "&" + f"msgdata={data.text}"
            name = url + "?" + params
            request_data = requests.post(url=name)
            if request_data.json().get('response').get('errorno') == '0':
                std.message_post(body=f'{data.text}', subtype='mt_comment')
            else:
                std.message_post(body='sms not sended', subtype='mt_comment')
            ...
            if sms.gateway_id.method == 'smpp':
                ...
                try:
                    ...
                    result = soap.telephonySmsUserSend(str(login), str(pwd),
                                                       ...
                                                       int(sms.priority), int(sms.coding), str(sms.gateway_id.tag),
...
class partner_sms_send(models.Model):
    ...
    _description = "Partner SSM Send"

    # @api.model
    # def _default_get_mobile(self):
    #     if self._context is None:
    #         self._context = {}
    #     partner_pool = self.env[self._context.get('active_model')]
    ...
    # mobile_to = fields.Many2one('odoocms.student', string='To', required=True, default=_default_get_mobile)
    ...
    def sms_send(self):
        ...
        for data in self:
            ...
            list_obj = self.env[self._context.get('active_model')].browse(self._context.get('active_ids'))
            for std in list_obj:
                msg_send_ids = client_obj.send_msg(std, data)
                status = msg_send_ids.json().get('response').get('errorno')
                if status == '0':
                    msg_send_list.append(status)
        if msg_send_list:
            message = '<h4 style="color:green;">Message has been send successfully<h4>'
        else:
            message = '<h4 style="color:red;">Message has been not send successfully<h4>'
        return self.message_wizard('Message', message)

        # To popup desired message on the screen

    def message_wizard(self, name=None, message=None):
        created_id = self.env['sms.wizard.message'].create({
            'text': (_(message))
        })
        return {
            'name': (_(name)),
