...
class OdooCMSCourseRegistrationBulk(models.Model):
    ...
    student_ids = fields.Many2many('odoocms.student', string='Students', required=True, states=SUBMITTED_STATES,
                                   tracking=True)
    term_id = fields.Many2one('odoocms.academic.term', 'Term', required=True, states=SUBMITTED_STATES, tracking=True)
    last_date = fields.Date(string='Registration Last Date', compute='get_registration_last_date', readonly=True,
                            store=True)
    ...
    override_max_limit = fields.Boolean('Override Maximum Limit?', default=False, states=SUBMITTED_STATES,
                                        tracking=True)

    compulsory_course_ids = fields.Many2many('odoocms.class.primary', 'class_course_compulsory_bulk2_rel',
                                             'register_id', 'primary_class_id',
                                             string="Compulsory Courses", states=SUBMITTED_STATES, tracking=True)
    elective_course_ids = fields.Many2many('odoocms.class.primary', 'class_course_elective_bulk2_rel', 'register_id',
                                           'primary_class_id',
                                           string="Elective Courses", states=SUBMITTED_STATES, tracking=True)
    repeat_course_ids = fields.Many2many('odoocms.class.primary', 'class_course_repeat_bulk2_rel', 'register_id',
                                         'primary_class_id',
                                         string="Repeat Courses", states=SUBMITTED_STATES, tracking=True)
    improve_course_ids = fields.Many2many('odoocms.class.primary', 'class_course_improve_bulk2_rel', 'register_id',
                                          'primary_class_id',
                                          string="Improve Courses", states=SUBMITTED_STATES, tracking=True)
    additional_course_ids = fields.Many2many('odoocms.class.primary', 'class_course_additional_bulk2_rel',
                                             'register_id', 'primary_class_id',
    ...
    def _get_courses_domain(self):
        ...
        if self.term_id:
            ...
            comp_course_ids = self.env['odoocms.study.scheme.line'].sudo().search(
                [('term_id', '=', self.term_id.id)]).filtered(lambda l: l.course_type == 'compulsory').mapped(
                'course_id')
            ...
            elective_course_ids = self.env['odoocms.study.scheme.line'].sudo().search(
                [('term_id', '=', self.term_id.id)]).filtered(lambda l: l.course_type == 'elective').mapped('course_id')
            elective_class_ids = self.env['odoocms.class.primary'].sudo().search([
                ('term_id', '=', self.term_id.id)]).filtered(
                lambda l: l.course_id.id in elective_course_ids.ids)

            # All repeat Courses
            repeat_course_ids = self.env['odoocms.study.scheme.line'].sudo().search(
                [('term_id', '=', self.term_id.id)]).mapped('course_id')
            repeat_class_ids = self.env['odoocms.class.primary'].sudo().search([
                ('term_id', '=', self.term_id.id)]).filtered(
                lambda l: l.course_id.id in repeat_course_ids.ids)

            # All improve Courses
            improve_course_ids = self.env['odoocms.study.scheme.line'].sudo().search(
                [('term_id', '=', self.term_id.id)]).mapped('course_id')
            ...
            # All repeat Courses
            additional_course_ids = self.env['odoocms.study.scheme.line'].sudo().search(
                [('term_id', '=', self.term_id.id)]).mapped('course_id')
    ...
    def action_submit(self):
        ...
        if not (
                self.compulsory_course_ids or self.elective_course_ids or self.repeat_course_ids or self.improve_course_ids or self.additional_course_ids):
            raise UserError('Please Select the Courses to Enroll!')

        students = self.student_ids.filtered(lambda l: all(
                tag.code not in ['withdrawal', 'suspension', 'suspension of registration'] for tag in l.tag_ids) and l.state2 == 'enroll')
    ...
    def action_approve(self):
        ...
        if all(registration.state == 'approved' for registration in self.registration_ids.sudo()):
            ...
            raise UserError('All Registrations are not in Approved State')
