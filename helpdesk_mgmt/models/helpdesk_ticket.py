...
class HelpdeskSubjectArea(models.Model):
    _name = "helpdesk.ticket.subject.area"
    _description = "Helpdesk Ticket Subject Area"

    name = fields.Char(string="Subject Area", required=True)
    # surface_area_id = fields.One2many('odoocms.faculty.portal.menu', 'parent_id', 'Child Menus')
...
class HelpdeskTicket(models.Model):
    ...
    def action_send_email(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('helpdesk_mgmt', 'helpdesk_email_template')[1]
        except ValueError:
            template_id = False
        try:
    ...
    user_id = fields.Many2one(comodel_name="res.users", string="Engineer name")
    subject_area = fields.Many2many('helpdesk.ticket.category', string="Subject Area")
    ...
    partner_name = fields.Char(string="User name")
    partner_email = fields.Char(string="Email")

    last_stage_update = fields.Datetime(
        string="Last status update", default=fields.Datetime.now
    )
